...
object FPFunctions {
    ...
    def filter[A](xs: List[A], f: A => Boolean): List[A] = xs match {
        case x :: rest => if(f(x)) x :: filter(rest, f) else filter(rest, f)
        case _ => Nil
    }
    ...
    def recFlat(xs: List[Any]): List[Any] = xs match {
        case x :: rest => x match {
            case l: List[_] => recFlat(l) ::: recFlat(rest)
            case _ => x :: recFlat(rest)
        }
        case _ => Nil
    }
 ...
 *
    ...
    def foldL[A, B](xs: List[A], f: (B, A) => B, init: B): B = xs match {
        case x :: rest => foldL(rest, f, f(init, x))
        case _ => init
    }
    ...
    def zip[A, B](xs: List[A], ys: List[B]): List[(A, B)] = xs match {
        case x :: xrest => ys match {
            case y :: yrest => (x,y) :: zip(xrest, yrest)
            case _ => Nil
        }
        case _ => Nil
    }
